 ...
 * Handles email, SMS, webhook, and in-app notifications
 */
...
app.get('/health', async (_req: Request, res: Response) => {
  const dbHealth = await notificationDatabase.healthCheck();
  ...
  const healthStatus = {
    ...
    dependencies: {
      database: dbHealth,
      email: {
        status: emailService.isReady() ? 'operational' : 'unavailable',
      },
      sms: {
        status: smsService.isReady() ? 'operational' : 'unavailable',
      },
      webhook: {
        status: webhookService.isReady() ? 'operational' : 'unavailable',
      },
    },
  };
...
async function initializeNotificationServices(): Promise<void> {
  try {
    logger.info('Initializing notification services...', { service: SERVICE_NAME });
    ...
    logger.info('Notification services initialized successfully', {
      ...
      sms: smsService.isReady(),
      webhook: webhookService.isReady(),
    });
  } catch (error) {
    ...
    const gracefulShutdown = async (signal: string) => {
      ...
      server.close(async () => {
        ...
        try {
          ...
          logger.info('Notification services closed', { service: SERVICE_NAME });
